...
from train_supcon import trainDMMR_supcon
...
def main(data_loader_dict, args, optim_config, cuda, writer, one_subject, seed=3):
    ...
    acc = trainDMMR_supcon(data_loader_dict, optim_config, cuda, args, iteration, writer, one_subject)
    return acc


if __name__ == "__main__":
    cuda = torch.cuda.is_available()
    parser = argparse.ArgumentParser(description="DMMR + SupCon")

    parser.add_argument("--way", type=str, default="DMMR_SUPCON/seed4", help="name of current way")
    parser.add_argument("--index", type=str, default="supcon_seed4_3subjects", help="tensorboard index")
    ...
    parser.add_argument("--start_subject", type=int, default=0, help="0-based held-out subject index to start from")
    parser.add_argument("--skip_existing", action="store_true", help="skip subjects whose model files already exist")
    ...
    parser.add_argument("--beta", type=float, default=0.05, help="balancing hyperparameter for adversarial loss")
    parser.add_argument("--lambda_con", type=float, default=0.1, help="balancing hyperparameter for supervised contrastive loss")
    parser.add_argument("--proj_hidden_dim", type=int, default=64, help="projection head hidden dim")
    parser.add_argument("--proj_output_dim", type=int, default=32, help="projection head output dim")
    ...
    start_subject = max(0, min(args.start_subject, args.subjects))
    subjects_to_run = min(args.max_subjects, args.subjects - start_subject)
    model_dir = os.path.join("model", args.way, args.index)

    for one_subject in range(start_subject, start_subject + subjects_to_run):
        if args.skip_existing:
            expected_files = [
                os.path.join(model_dir, "{}_pretrain_model.pth".format(one_subject)),
                os.path.join(model_dir, "{}_tune_model.pth".format(one_subject)),
                os.path.join(model_dir, "{}_test_model.pth".format(one_subject)),
            ]
            if all(os.path.exists(path) for path in expected_files):
                print("skip subject {} because model files already exist".format(one_subject))
                continue
